...
<form wire:submit.prevent="save">
    <x-modal.dialog class="w-20" wire:model.defer="showModal">
        ...
        <x-slot name="content">
            <x-input.group for="name" label="Name/Keywords" :error="$errors->first('editing.name')">
                <x-input wire:model="editing.name" id="name" type="text" />
                <div class="text-xs mt-1">Use a space-separated list of words for search.</div>
            </x-input.group>
            ...
            <x-input.group label="Used by">
                    <ul class="gap-x-5 gap-y-1 leading-tight grid grid-cols-4 text-xs">
                        @foreach(Item::where('item_icon_id', $editing->id)->paginate($pageSize) as $item)
                            <li><a class="link" href="{{route('items')}}?filters[id]={{$item->id}}">{{ $item->name }}</a></li>
                        @endforeach
                    </ul>
            @if($editing->items->count() > $pageSize)
                <div class="text-base text-center">and {{ $editing->items->count() - $pageSize }} more</div>
                @endif
            </x-input.group>
            @endisset
