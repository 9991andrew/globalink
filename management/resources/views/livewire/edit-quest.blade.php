            ...
            @isset($editing->id)
                ...
                @if(isset($editing->questResultType) && $editing->quest_result_type_id == 16)
                    <x-input.group label="Conversation URL">
                        <textarea class="w-full mb-2" wire:model.lazy="question" placeholder="https://conversation.megaworld.game-server.ca/task.php?conversation_id=123">
                            @php if (!$this->question && isset($editing->questAnswers()->get()[0]))
                                     $this->question = $editing->questAnswers()->get()[0]->question
                            @endphp
                        </textarea>
                    </x-input.group>
