<?php
/**
 * Guardian Creation view
 */
$pageTitle = _("Edit Guardian Avatar");
...
// Make assoc array of guardian avatar images with typeid as key
// This allows us to access all the information about the guardian's current avatar
$guardianImages = array();

foreach($guardian->fetchGuardianAvatar($player->getId()) as $guardianAvatarItems) {
    $imageInfo = array();
    $imageInfo['aid'] = $guardianAvatarItems->getId();
    $imageInfo['color1'] = $guardianAvatarItems->getColor1();
    $imageInfo['color2'] = $guardianAvatarItems->getColor2();
    $imageInfo['color3'] = $guardianAvatarItems->getColor3();
    $imageInfo['color4'] = $guardianAvatarItems->getColor4();
    $guardianImages[$guardianAvatarItems->getAvatarImageTypeId()] = $imageInfo;
}
// echo '<pre><code>'.var_dump($guardianImages).'</code></pre>';

echo '<h2 class="mb-2 mt-8 text-xl font-ocr text-center">'.$pageTitle.'</h2>';

echo '<h3 class="mt-4 mb-6 text-center"><div class="banner">Guardian</div></h3 class="text-center">';

echo '<div class="flex justify-center flex-wrap">';

echo '<div class="w-full xs:w-80">';
	echo $guardian->showGuardianAvatar($player->getId());
echo '</div>';

echo '<form class="sm:w-80" action="guardian_edit_action.php" method="post">';
...
foreach ($itemTypes as $itemType) {
	$typeId = $itemType['id'];
	$imageList = AvatarImage::getAvatarImages($typeId, $guardian->getGenderId($player->getId()), $guardian->getRaceId($player->getId()));
        ...
        <div class="xs:grid xs:grid-cols-3 xs:items-center py-1">
			...
			<div class="itemPicker mt-1 xs:w-52 xs:mt-0 xs:col-span-2 flex items-center space-x-3">
				...
				<select id="itemType<?=$typeId?>" <?php echo ($typeId==1)?'autofocus':''?> name="itemType<?=$typeId?>" class="flex-1 w-full">
					<?php
					// I need to determine if the guardian has any of these items.
					// Could check the existing array of itemsEquipped.
					// Could make a function to test of the guardian has a certain itemid
					// Could make a function to determine which items a guardian has of a certain category?
					foreach ($imageList as $image) {
						$s="";
						if ($guardianImages[$typeId]['aid'] == $image->getId()) $s = ' selected="selected" ';
            ...
            if (sizeof($colors)) {
                for ($i=1; $i<=4; $i++) {
                    ...
                    echo '<input type="hidden"
                        ...
                        value="'.(isset($guardianImages[$typeId])?$guardianImages[$typeId]['color'.$i]:'').'" />';
                    foreach ($colors as $color) {
                        $s = "";
                        if ( isset($guardianImages[$typeId]) && $guardianImages[$typeId]['color'.$i] == $color ) {
                            $s=" selected";
                        }
...
?>
	...
	<div class="my-8 flex justify-around">
		...
		<a class="btn" id="submitAvatar" href="map.php"><?=_('Cancel')?></a>
	</div>
...
const avatarImages=<?php
    ...
    foreach(AvatarImage::getAvatarImageTypes() as $type) {
        $images = AvatarImage::getAvatarImages($type['id'], $guardian->getGenderId($player->getId()), $guardian->getRaceId($player->getId()));
        $imagesArray = array();
